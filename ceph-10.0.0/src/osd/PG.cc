 ...
 * 同时将收到的OSD信息存在@peer_info以及@might_have_unfound中。
 */
...
/*
 * OyTao: primary OSD向其他probe osd发送pg_info_query消息。
 * 该函数有可能在收到probe osd的ACK消息之后再次被调用。prior_set可能发生变化。
 * (GetInfo::react)
 */
...
boost::statechart::result PG::RecoveryState::GetInfo::react(const MNotifyRec& infoevt) 
{
	...
	/* 
	 * OyTao: 将收到的OSD从@peer_info_requested和@blocked_by数组中删除
	 */
	...
	if (pg->proc_replica_info(
		 ...
		 * @history.last_epoch_started 可能在proc_replica_info中与收到OSD pg_info
		 * 中的history info信息merge, 更新当前history中的信息为最新的。
		 * 如果@last_epoch_started发生过改变，则表示前期确定的past intervals
		 * 需要重新build.
		 */
		if (old_start < pg->info.history.last_epoch_started) {
			...
			/* 
			 * OyTao: 根据新生成的prior set,将不需要再peering的OSD从@peer_info_requested
			 * 中删除,再次get_infos.
			 */
			...
			/* OyTao: 是否可以不需要调用? 重新确认blocked_by数组 ? */
			get_infos();
		}
		...
		/*
		 * OyTao: @peer_info_requested.empty 表示都已经收到所有的OSD的peer info.
		 * pg_down == Flase,表示PG状态正常。(如果past intervals中某一个interval无法进行
		 * 数据恢复，则pg_down = True)
		 */
		if (peer_info_requested.empty() && !prior_set->pg_down) {
			/*
			...
			if (pg->info.history.last_epoch_started) {
				for (map<epoch_t,pg_interval_t>::reverse_iterator p = pg->past_intervals.rbegin();
					...
					/*
					 * OyTao: 遍历@interval中acting osd set.找出其中至今UP并且
					 * complete的OSD。如果不存在(any_up_complete_now == false)
					 * 则该interval中可能会丢失数据，导致PG无法恢复。设置
					 * PG的state == PG_STATE_DOWN. 直接退出。
					 */
					for (unsigned i=0; i<interval.acting.size(); i++) {
						int o = interval.acting[i];
						if (o == CRUSH_ITEM_NONE)
